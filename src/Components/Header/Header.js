...
const Header = ({ accessToken, userProfile, logOut }) => {
    ...
    return (
        ...
        <div className="rounded bg-black w-full h-16 py-4 opacity-70 relative">
            ...
            {accessToken &&
                <>
                    <img className="rounded-full w-8 h-8 sm:w-10 sm:h-10 absolute right-2 top-2" src={userProfile?.images[0].url}></img>
                    <button className="py-1 pr-3  text-white absolute right-2  top-16 bg-[rgb(0,0,0,0.7)] flex items-center rounded" onClick={logOut}>
